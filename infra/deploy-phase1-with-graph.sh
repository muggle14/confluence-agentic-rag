...
# Phase 1: Deploy Azure AI Search with Graph Enrichment and Native Features
set -e
...
echo -e "${BLUE}==================================================${NC}"
echo -e "${BLUE}🚀 Phase 1: Graph-Aware Search with Azure Native Features${NC}"
echo -e "${BLUE}==================================================${NC}"
...
echo -e "\n${YELLOW}🗄️ Step 4: Creating Index with Graph Fields...${NC}"
cat > index.json << EOF
{
  "name": "$SEARCH_INDEX",
  "fields": [
...
cat > test-search.json << EOF
{
  ...
  "queryType": "simple",
  "searchFields": "title,content,parent_page_title,hierarchy_path",
