...
# This version uses REST API calls instead of Azure CLI for Search operations
set -e
...
echo -e "\n${YELLOW}🔑 Getting Search Service Admin Key...${NC}"
...
if [ -z "$SEARCH_KEY" ]; then
    echo -e "${RED}Failed to get search service key!${NC}"
    exit 1
fi

# Get Storage Connection String
echo -e "\n${YELLOW}🔑 Getting Storage Connection String...${NC}"
...
# Step 2: Create Data Source using REST API
...
# Create datasource using REST API
curl -X PUT \
  "${SEARCH_ENDPOINT}/datasources/confluence-blob-datasource?api-version=${API_VERSION}" \
  -H "Content-Type: application/json" \
  -H "api-key: ${SEARCH_KEY}" \
  -d @datasource.json

echo -e "\n${GREEN}✅ Data source created${NC}"
...
curl -X PUT \
  "${SEARCH_ENDPOINT}/skillsets/confluence-graph-skillset?api-version=${API_VERSION}" \
  -H "Content-Type: application/json" \
  -H "api-key: ${SEARCH_KEY}" \
  -d @skillset.json

echo -e "\n${GREEN}✅ Skillset created${NC}"
...
curl -X PUT \
  "${SEARCH_ENDPOINT}/indexes/${SEARCH_INDEX}?api-version=${API_VERSION}" \
  -H "Content-Type: application/json" \
  -H "api-key: ${SEARCH_KEY}" \
  -d @index.json

echo -e "\n${GREEN}✅ Index created${NC}"

# Step 5: Create Indexer
echo -e "\n${YELLOW}⚙️ Step 5: Creating Indexer...${NC}"
cat > indexer.json << EOF
{
  ...
  "fieldMappings": [
    ...
    {
      "sourceFieldName": "chunk_type",
      "targetFieldName": "chunk_type"
    },
    {
    ...
    {
      "sourceFieldName": "total_chunks",
      "targetFieldName": "total_chunks"
    },
    {
      "sourceFieldName": "content_vector",
      "targetFieldName": "content_vector"
    },
    {
      "sourceFieldName": "title_vector",
      "targetFieldName": "title_vector"
    },
    {
...
curl -X PUT \
  "${SEARCH_ENDPOINT}/indexers/confluence-graph-indexer?api-version=${API_VERSION}" \
  -H "Content-Type: application/json" \
  -H "api-key: ${SEARCH_KEY}" \
  -d @indexer.json

echo -e "\n${GREEN}✅ Indexer created${NC}"

# Step 6: Run the indexer
echo -e "\n${YELLOW}🏃 Step 6: Running Indexer...${NC}"
curl -X POST \
  "${SEARCH_ENDPOINT}/indexers/confluence-graph-indexer/run?api-version=${API_VERSION}" \
  -H "api-key: ${SEARCH_KEY}"

echo -e "\n${GREEN}✅ Indexer started${NC}"
...
check_indexer_status() {
    local status_response=$(curl -s -X GET \
        "${SEARCH_ENDPOINT}/indexers/confluence-graph-indexer/status?api-version=${API_VERSION}" \
        -H "api-key: ${SEARCH_KEY}")
    
    echo "$status_response" | jq -r '.lastResult.status // "unknown"'
}

# Monitor for a few iterations
for i in {1..5}; do
    sleep 10
    status=$(check_indexer_status)
    echo "Indexer status: $status"
    
    if [ "$status" == "success" ]; then
        echo -e "${GREEN}✅ Indexer completed successfully!${NC}"
        break
    elif [ "$status" == "inProgress" ]; then
        echo "Indexer is still running..."
    else
        echo -e "${YELLOW}⚠️ Indexer status: $status${NC}"
    fi
done
...
echo -e "\n${BLUE}==================================================${NC}"
echo -e "${GREEN}✅ Phase 1 Enhanced Deployment Complete!${NC}"
echo -e "${BLUE}==================================================${NC}"
echo -e "\nNext steps:"
echo -e "1. Monitor the indexer progress in Azure Portal"
echo -e "2. Verify documents are being enriched with graph data"
echo -e "3. Test search queries with graph-aware ranking"
echo -e "4. Run test-phase1-deployment.sh to validate"

# Display some useful commands
echo -e "\n${YELLOW}Useful Commands:${NC}"
echo -e "Check indexer status:"
echo -e "  curl -X GET '${SEARCH_ENDPOINT}/indexers/confluence-graph-indexer/status?api-version=${API_VERSION}' -H 'api-key: ${SEARCH_KEY}' | jq"
echo -e "\nSearch documents:"
echo -e "  curl -X POST '${SEARCH_ENDPOINT}/indexes/${SEARCH_INDEX}/docs/search?api-version=${API_VERSION}' -H 'api-key: ${SEARCH_KEY}' -H 'Content-Type: application/json' -d '{\"search\": \"*\", \"top\": 5}' | jq"