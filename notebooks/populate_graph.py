...
# Local imports
from .config import GraphConfig, ContainerNames, NodeTypes, EdgeTypes
from .graph_models import (
    PageNode, SpaceNode, LinkNode, 
    GraphModelFactory, validate_node_data
)
from .graph_operations import GraphOperations
from .utils import ProgressTracker, DataValidator, GraphAnalyzer
...
class GraphPopulator:
    ...
    def _create_space_nodes(self, processed_pages: List[Dict[str, Any]]) -> None:
        ...
        # Batch create spaces
        if space_nodes:
            results = self.graph_ops.batch_create_nodes(space_nodes)
            self.stats['spaces_created'] = results['success']
            print(f"✅ Created {results['success']} space nodes")
    ...
    def _create_page_nodes(self, processed_pages: List[Dict[str, Any]]) -> None:
        ...
        for page_data in processed_pages:
            try:
                ...
                print(f"⚠️ Error creating page node for {page_data.get('pageId', 'unknown')}: {e}")
                self.stats['warnings_count'] += 1
        ...
        # Batch create pages
        if page_nodes:
            results = self.graph_ops.batch_create_nodes(page_nodes)
            self.stats['pages_processed'] = results['success']
            print(f"✅ Created {results['success']} page nodes in graph")
    ...
    def _create_link_nodes(self, processed_pages: List[Dict[str, Any]]) -> None:
        ...
        for page_data in processed_pages:
            ...
            for link in links:
                ...
                if link_type in ['external', 'email'] and url:
                    link_key = url
                    if len(link_key) > 50:
                        import hashlib
        ...
        # Batch create links
        if link_nodes:
            results = self.graph_ops.batch_create_nodes(link_nodes)
            self.stats['links_created'] = results['success']
            print(f"✅ Created {results['success']} link nodes")
    ...
    def _create_relationships(self, processed_pages: List[Dict[str, Any]]) -> None:
        ...
        for page_data in processed_pages:
            try:
                ...
                print(f"⚠️ Error creating relationships for page {page_data.get('pageId', 'unknown')}: {e}")
                self.stats['warnings_count'] += 1
        ...
        # Batch create edges
        if all_edges:
            results = self.graph_ops.batch_create_edges(all_edges)
            self.stats['edges_created'] = results['success']
            print(f"✅ Created {results['success']} relationships in graph")
    ...
    def _process_changed_pages(self, changed_pages: List[Dict[str, Any]]) -> None:
        ...
        for page_data in changed_pages:
            try:
                ...
                print(f"⚠️ Error processing changed page {page_data.get('pageId', 'unknown')}: {e}")
                self.stats['warnings_count'] += 1
        ...
        if page_nodes:
            results = self.graph_ops.batch_create_nodes(page_nodes)
            self.stats['pages_processed'] = results['success']
            print(f"✅ Updated {results['success']} page nodes")
    ...
    def _update_relationships(self, changed_pages: List[Dict[str, Any]]) -> None:
        ...
        for page_data in changed_pages:
            try:
                ...
                print(f"⚠️ Error updating relationships for {page_data.get('pageId', 'unknown')}: {e}")
                self.stats['warnings_count'] += 1
        ...
        if all_edges:
            results = self.graph_ops.batch_create_edges(all_edges)
            self.stats['edges_created'] = results['success']
            print(f"✅ Updated {results['success']} relationships")
    
    def _validate_graph(self) -> None:
        """Validate graph integrity"""
        print("🔍 Validating graph integrity...")
        
        validation_results = self.graph_ops.validate_graph_integrity()
...
def main():
    ...
    try:
        ...
        if results['success']:
            ...
            print(f"❌ Graph population failed: {results['error']}")
            sys.exit(1)
